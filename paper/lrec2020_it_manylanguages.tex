...
\address{$^1$Department of Linguistics, University of Potsdam \\  
$^2$ Department of Linguistics, Uppsala University \\
$^3$ Department of Philosophy, Linguistics and Theory of Science, University of
Gothenburg  \\ 
...
Slovakia became the 16th member of the euro area. My country has surrendered a
...
2009, la Slovaquie est devenue le 16e membre de la zone euro. Mon pays a
...
%event  &&&&&&&&&&& \\ it  &  --  &&&&&&&&&&& \\ \midrule
\multicolumn{14}{c}{\textbf{Features}}\\ \midrule DE & FI & SV & IT &LV& NL &
HU&PL & SL & PT &  SK& RO& ET& ES  \\ &&&&&&&&&&&&&\\ \textit{dies}&\textit{on}&
\textit{det}& \textit{e}& \textit{tas}& \textit{het}& \textit{hogy}&
\textit{pre}& \textit{je}& empty& \textit{'}& empty& \textit{on}&
\textit{desgracia} \\ &&&&&&&&&&&&&\\ 
 \textit{das}&\textit{puhua}& \textit{det}& \textit{-}& \textit{ir}&
 \textit{sprake}& \textit{lehetsége}& \textit{sa}& \textit{prav}& \textit{soar}&
 \textit{' }& \textit{în}& empty& \textit{puede} \\
 &&&&&&&&&&&&&\\ \textit{es} &\textit{ei}& empty& \textit{non}&
 \textit{nepietiks}& empty& \textit{nem}& empty& \textit{zgolj}& empty&
 \textit{'}& \textit{nu}& \textit{vaeste}& \textit{no}  \\ 
 \bottomrule
